...
if sys.argv[1] == "push":
    ...
    status, output = outcomes[index].split(":", 1)
...
class PushTapTest(unittest.TestCase):
    def run_helper(self, outcomes: str) -> tuple[subprocess.CompletedProcess[str], list[list[str]]]:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                env=os.environ | {"PATH": f"{root}{os.pathsep}{os.environ['PATH']}", "FAKE_GIT_LOG": str(log), "PUSH_OUTCOMES": outcomes},
            )
    ...
    def test_retry_exhaustion_stops_after_three_pushes(self) -> None:
        ...
        self.assertEqual([command[0] for command in commands], ["fetch", "rebase", "push"] * 3)
